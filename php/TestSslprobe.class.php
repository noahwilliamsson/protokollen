<?php
/**
 * Handle JSON generated by the sslprobe tool
 */
...
class TestSslprobe extends ServiceGroup {
	...
	/**
	 * Lookup current sslprobe
	 * @param $svcId Service ID
	 * @param $svcGrpId Service group ID
	 * @param $hostname Hostname
	 * @returns Object or NULL, throws on errors
	 */
	function getItem($svcId, $svcGrpId, $hostname) {
		$m = $this->getMySQLHandle();

		$q = 'SELECT * FROM test_sslprobes
			WHERE service_id=? AND svc_group_id=?
			AND entry_type="current"
			AND hostname=?';
		if(($st = $m->prepare($q)) === FALSE) {
			$err = __METHOD__ .": Lookup"
				." ($svc->id, $svcGrpId, $hostname) failed:"
				." $m->error";
			throw new Exception($err);
		}

		$st->bind_param('iis', $svcId, $svcGrpId, $hostname);
		if($st->execute() === FALSE) {
			$err = __METHOD__ .": Lookup"
				." ($svc->id, $svcGrpId, $hostname) failed:"
				." $m->error";
	...
	/**
	 * Import JSON from the sslprobe tool
	 * @param $svcId Service ID
